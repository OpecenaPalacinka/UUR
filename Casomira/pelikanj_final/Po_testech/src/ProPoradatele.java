...
public class ProPoradatele extends Application {
    ...
    int minCislo = 1;
    int maxCislo = 99;
    boolean pomocnaNaVylouceni = true;
    static boolean pauza = true;
    Scene scene2;
    int pocitadloProHistorii = 0;
    public static int pocitadloProDivaky = 0;
    static Hrac1[] kopie = new Hrac1[0];
    static List<Hrac1> list = new ArrayList<>(Arrays.asList(kopie));
    static Hrac2[] kopie2 = new Hrac2[0];
    static List<Hrac2> list2 = new ArrayList<>(Arrays.asList(kopie2));
    static CasATretina param = new CasATretina(30,0);
    static CasATretina param2 = new CasATretina(30,0);
    CheckBox timeoutTym2;
                    ...
                    if(pauza){
                        ...
                        try {
                            for (Hrac1 hrac: tableNaVylouceni21.getItems()
                                ...
                                if(hrac.getVylouceni().getVteriny() == 0 && hrac.getVylouceni().getMinuty() == 0){
                                    list.add(hrac);
                                    kopie = list.toArray(new Hrac1[0]);
                                    tableNaVylouceni21.getItems().remove(hrac);
                                }
                            }
                            for (Hrac1 hrac: tableNaVylouceni51.getItems()
                                ...
                                if(hrac.getVylouceni().getVteriny()==0 && hrac.getVylouceni().getMinuty() ==0){
                                    list.add(hrac);
                                    kopie = list.toArray(new Hrac1[0]);
                                    tableNaVylouceni51.getItems().remove(hrac);
                                }
                            }
                            for (Hrac2 hrac: tableNaVylouceni22.getItems()
                                ...
                                if(hrac.getVylouceni().getVteriny()==0 && hrac.getVylouceni().getMinuty() ==0){
                                    ...
                                    tableNaVylouceni22.getItems().remove(hrac);
                                }
                            }
                            for (Hrac2 hrac: tableNaVylouceni52.getItems()
                                ...
                                if(hrac.getVylouceni().getVteriny()==0 && hrac.getVylouceni().getMinuty() ==0){
                                    list2.add(hrac);
                                    kopie2 = list2.toArray(new Hrac2[0]);
                                    tableNaVylouceni52.getItems().remove(hrac);
                                }
                            }
    ...
    public Node createRight(){
        ...
        table2.setPlaceholder(new Label("Žádní hráči na soupisce"));
        ...
        addBT.setOnAction(actionEvent -> {
            boolean pridaniHraceBoolean = true;
            ...
            for (Hrac2 hrac: hrac2ObservableList
                 ) {
                if(hrac.getCisloHrace() == Integer.parseInt(pridaniCislo.getText())){
                    Alert alert = new Alert(Alert.AlertType.WARNING);
                    alert.setHeaderText("Špatné číslo");
                    alert.setContentText("Hráč se zadaným číslem už existuje!");
                    pridaniCislo.setText("");
                    pridaniHraceBoolean = false;
                    alert.showAndWait();
                }
            }
            if(pridaniHraceBoolean){
    ...
    public Node createMid() {
        ...
        Label timeoutCas = new Label();
        timeoutTym1.selectedProperty().bindBidirectional(tym1.timeoutProperty());
        timeoutTym1.setOnAction(actionEvent -> {
            if(pauza && timeoutTym1.isSelected()){
                tym1.setTimeout(true);
                timeoutTym1.setDisable(true);
                timeoutTym2.setDisable(true);
                start.setDisable(true);
                stop.setDisable(true);
                timeoutCas.textProperty().bind(Bindings.createStringBinding(()-> (param.getMinuty() +":"+param.getVteriny())
                        ,param.minutyProperty(),param.vterinyProperty()));
                ...
                timer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        Platform.runLater(()-> {
                            param.odectiVterinu();
                            if (param.getVteriny() == 0){
                                timer.cancel();
                                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                                alert.setTitle("Konec");
                                alert.setHeaderText("Timeout týmu 1 skončil");
                                alert.setContentText("Timeout týmu 1 skončil");
                                alert.showAndWait();
                                if(!timeoutTym2.isSelected()){
                                    timeoutTym2.setDisable(false);
                                }
                                start.setDisable(false);
                                stop.setDisable(false);
                                timeoutCas.textProperty().unbind();
                                timeoutCas.textProperty().setValue("");
                            }
                        });
                    }
                },0,1000);
            } else {
                timeoutTym1.setSelected(false);
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("Timeout");
                alert.setHeaderText("Nemůžete zapnout timeout");
                alert.setContentText("Běží čas, nemůžete zapnout timeout!");
                alert.showAndWait();
            }
        });
        Label timeout = new Label("Timeout");
        timeoutTym2 = new CheckBox();
        ...
        timeoutTym2.setOnAction(actionEvent -> {
            if(pauza && timeoutTym2.isSelected()) {
                tym2.setTimeout(true);
                timeoutTym2.setDisable(true);
                timeoutTym1.setDisable(true);
                start.setDisable(true);
                stop.setDisable(true);
                timeoutCas2.textProperty().bind(Bindings.createStringBinding(()-> (param2.getMinuty() +":"+param2.getVteriny())
                        ,param2.minutyProperty(),param2.vterinyProperty()));
                Timer timer = new Timer();
                timer.scheduleAtFixedRate(new TimerTask() {
                    @Override
                    public void run() {
                        Platform.runLater(()->{
                            param2.odectiVterinu();
                            if (param2.getVteriny() == 0){
                                timer.cancel();
                                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                                alert.setTitle("Konec");
                                alert.setHeaderText("Timeout týmu 2 skončil");
                                alert.setContentText("Timeout týmu 2 skončil");
                                alert.showAndWait();
                                if(!timeoutTym1.isSelected()){
                                    timeoutTym1.setDisable(false);
                                }
                                start.setDisable(false);
                                stop.setDisable(false);
                                timeoutCas2.textProperty().unbind();
                                timeoutCas2.textProperty().setValue("");
                            }
                        });
                    }
                },0,1000);
            } else {
                timeoutTym2.setSelected(false);
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("Timeout");
                alert.setHeaderText("Nemůžete zapnout timeout");
                alert.setContentText("Běží čas, nemůžete zapnout timeout!");
        ...
        timeouty.setSpacing(70);
        timeouty.getChildren().addAll(timeoutCas,timeoutTym1,timeout,timeoutTym2,timeoutCas2);

        tableNaVylouceni21.setEditable(false);
        tableNaVylouceni21.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);
        tableNaVylouceni21.setPlaceholder(new Label("Žádné vyloučení"));
        ...
        tableNaVylouceni22.setPlaceholder(new Label("Žádné vyloučení"));
        ...
        tableNaVylouceni51.setPlaceholder(new Label("Žádné vyloučení"));
        ...
        tableNaVylouceni52.setPlaceholder(new Label("Žádné vyloučení"));
            ...
            try {
                switch (tymChoice.getValue()) {
                    case Tym1:
                        ...
                        if(pomocnaNaVylouceni){
                            Alert alert = new Alert(Alert.AlertType.WARNING);
                            alert.setTitle("Varování");
                            alert.setHeaderText("Vyloučení");
                            alert.setContentText("Hráč s číslem "+pridaniCislo.getText()+" není na soupisce a nebo už je vyloučený.");
                            alert.showAndWait();
                        }
                        break;
                            ...
                            if (!tableNaVylouceni22.getItems().contains(hraci)
                                ...
                                if (Integer.parseInt(pridaniCislo.getText()) == hraci.getCisloHrace()) {
                                    if (vylouceniTyp.getValue() == TypVylouceni.DVE) {
                                        ...
                                        list2.remove(hraci);
                                        kopie2 = list2.subList(0, list2.size()).toArray(new Hrac2[0]);
                                        pomocnaNaVylouceni = false;
                                        break;
                                    }
                                    if (vylouceniTyp.getValue() == TypVylouceni.PET) {
                ...
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("Varování");
                alert.setHeaderText("Tým/Číslo hráče/Doba vyloučení");
                alert.setContentText("Vyberte tým, zadejte číslo hráče nebo dobu vyloučení!");
                alert.showAndWait();
            }
            pridaniCislo.setText("");
            tymChoice.setValue(null);
            vylouceniTyp.setValue(null);
    ...
    public Node createLeft(){
        ...
        table1.setPlaceholder(new Label("Žádní hráči na soupisce"));
        ...
        Button addBT = new Button("Přidat");
        addBT.setOnAction(actionEvent -> {
            boolean pridaniHraceBoolean2 = true;
            if (pridaniCislo.getText().equals("")) {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setHeaderText("Špatné číslo");
                alert.setContentText("Nezadali jste číslo. Zadejte číslo mezi 1 - 99!");
                pridaniHraceBoolean2 = false;
                alert.showAndWait();
            } else if (pridaniJmeno.getText().equals("")) {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setHeaderText("Nezadali jste jméno");
            ...
            for (Hrac1 hrac: hrac1ObservableList
            ) {
                if(hrac.getCisloHrace() == Integer.parseInt(pridaniCislo.getText())){
                    Alert alert = new Alert(Alert.AlertType.WARNING);
                    alert.setHeaderText("Špatné číslo");
    ...
    public MenuBar createMenu(){
        ...
        ulozitZapas.setOnAction(actionEvent -> {
            ...
            String jmenoTymu1 = tym1.getJmenoTymu();
            String jmenoTymu2 = tym2.getJmenoTymu();
            try {
                if(jmenoTymu1.equals("")){
                    jmenoTymu1 = "Tým 1";
                }
                if (jmenoTymu2.equals("")){
                    jmenoTymu2 = "Tým 2";
                }
            } catch (Exception ignored){
            }

            jmenaTymu.setValue(jmenoTymu1 + "  vs  " + jmenoTymu2);
    ...
    private void vypsaniNapovedy(){
        ...
        alert.setContentText("Jako první doporučuji vyplnit jména obou týmů, kolonky najdete po stranách" +
                ...
                " nejdřív vyberete tým který bude vyloučený, poté hráče a pak dobu trestu. Okno pro diváky se otevírá " +
                "v menu, kde kliknete na první záložku a poté vyberete možnost \"Zobrazení pro diváky\". Zápas se může" +
                "uložit v menu Hra-> Zápas-> Uložit zápas a uložené zápasy zobrazit v Historii zápasů.");
